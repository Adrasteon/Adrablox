...
try {
    ...
    Write-Output 'Ignored error (empty catch) in run_mcp_invalid_session_task.ps1'
}
    }
    ...
    Write-Output "Server is healthy. Running invalid-session contract test..."
    & (Join-Path $workspace 'tools\mcp_invalid_session_contract_test.ps1')

    Write-Output "Invalid-session task completed successfully."
}
finally {
