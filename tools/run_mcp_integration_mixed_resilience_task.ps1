...
try {
    ...
    Write-Output 'Ignored error (empty catch) in run_mcp_integration_mixed_resilience_task.ps1'
}
    }
    ...
    Write-Output "Server is healthy. Running integration mixed-resilience contract test..."
    & (Join-Path $workspace 'tools\mcp_integration_mixed_resilience_contract_test.ps1') -Iterations $Iterations

    Write-Output "Integration mixed-resilience task completed successfully."
}
finally {
