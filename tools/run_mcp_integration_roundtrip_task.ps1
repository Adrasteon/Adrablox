...
try {
    ...
    Write-Output 'Ignored error (empty catch) in run_mcp_integration_roundtrip_task.ps1'
}
    }
    ...
    Write-Output "Server is healthy. Running integration roundtrip contract test..."
    & (Join-Path $workspace 'tools\mcp_integration_roundtrip_contract_test.ps1')

    Write-Output "Integration roundtrip task completed successfully."
}
finally {
