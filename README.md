...
- Tooling and tasks
  ...
  - Manual release packaging workflow: `.github/workflows/release-packaging.yml` (`workflow_dispatch` only; packages server and plugin artifacts for Windows/Linux/macOS, installs pinned Rojo `7.7.0-rc.1`, enforces installable plugin build, validates manifest/artifact naming, runs packaged Day-0 validation, and generates/verifies release checksums)
