...
- Tooling and tasks
  ...
  - CI workflow: `.github/workflows/ci.yml` (Windows expanded contract checks + Linux/macOS protocol contract checks)
    ...
    - Optional release-candidate evidence gate: set `workflow_dispatch` input `run_release_candidate_evidence_pack=true` to install pinned Rojo, run `tools/run_release_candidate_evidence_pack_task.ps1`, and upload input-aware evidence artifacts (`release-candidate-evidence-it<iterations>-dist-on|off`) containing reliability + parity + readiness outputs.
