...
- Tooling and tasks
  ...
  - `tools/package_release_artifacts.ps1`: the canonical repository packager. It builds the release server binary, creates a versioned plugin source archive, and (when the Rojo CLI is present and not explicitly skipped) builds an installable plugin `.rbxm`. Prefer this script for CI and manual packaging; Rojo project JSON edits are experimental and moved to `plugin/experimental/mcp-studio.plugin.project.json`.
    - Canonical no-Rojo plugin hierarchy (Studio-compatible): `AdrabloxMCP_PluginBuilder/AdrabloxMCP/{MainScript,src,ui}` as sibling children under `AdrabloxMCP`.
    - Canonical entrypoint behavior: `plugin/mcp-studio/packaging/PluginMain.server.lua` resolves modules from sibling `src` via `script.Parent:WaitForChild("src")`.
  ...
  - CI workflow: `.github/workflows/ci.yml` (Windows expanded contract checks + Linux/macOS protocol contract checks)
    - Always-on packaging gate (push/PR): Windows runs `tools/package_release_artifacts.ps1 -SkipServerBuild -SkipRojo -OutputDir dist/ci-no-rojo` to validate the no-Rojo plugin packaging path. This step fails fast if generated plugin XML regresses to legacy script-source encoding/layout (for example `<string name="Source">` or stale `__files` structure).
    ...
    - Optional reliability evidence gate: set `workflow_dispatch` input `run_integration_reliability_suite=true` to run `tools/run_mcp_integration_reliability_suite_task.ps1` and upload `tools/integration_reliability_report.json` as workflow artifact `integration-reliability-report-it<iterations>`.
    - Optional readiness gate: set `workflow_dispatch` input `run_spec_readiness_report=true` to generate `tools/spec_readiness_report.json` and upload it as workflow artifact `spec-readiness-report-standalone` (or `spec-readiness-report-with-pack` when run alongside evidence-pack flow).
    - Optional release-candidate evidence gate: set `workflow_dispatch` input `run_release_candidate_evidence_pack=true` to run `tools/run_release_candidate_evidence_pack_task.ps1` (which uses the canonical `tools/package_release_artifacts.ps1` for distribution packaging). The parity checks still rely on the Rojo CLI when available on the runner; CI will not automatically install Rojo for packaging.
    ...
    - Mission-critical strict mode: `release_candidate_fail_if_not_pass=true` (default) makes the evidence-pack run fail unless `specComplete=PASS`.
  ...
  - Manual release packaging workflow: `.github/workflows/release-packaging.yml` (`workflow_dispatch` only). This workflow packages server and plugin artifacts for Windows/Linux/macOS using `tools/package_release_artifacts.ps1`. The workflow will only install Rojo on the runner if an explicit packaging step requires it; the canonical flow does not depend on Rojo being present.
