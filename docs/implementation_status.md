...
- Validation/tooling
  ...
  - Mission-critical CI dispatch helper now enforces strict non-zero `gh` exit handling so unauthenticated/failed dispatches fail fast.
  ...
  - Plugin installable packaging project: `plugin/mcp-studio.plugin.project.json` (Rojo build target for versioned `.rbxm` output when available).
  ...
  - Day-0 published-bundle validation script: `tools/run_day0_published_artifact_validation_task.ps1` (runs packaged server binary from release bundle with temp project and direct MCP lifecycle/tool checks).
  ...
  - GitHub Actions CI (`.github/workflows/ci.yml`) runs:
    - Windows: tests + smoke + policy contract + Rojo compatibility + Rojo changefeed edge-case + conflict race + reconnect/replay + invalid-session + integration roundtrip + integration reconnect-loop + integration conflict-recovery + integration mixed-resilience + protocol contract checks,
  ...
  - Manual release packaging workflow now also runs published-artifact Day-0 validation (with installable plugin required).
